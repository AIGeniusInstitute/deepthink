.PHONY: dev dev-backend dev-web build build-backend build-web start start-prod start-isolated \
       ...
       sandbox-build logs status stop stop-prod stop-isolated \
...
stop-prod: ## 停止 9999 端口的生产实例（复用 stop 逻辑，pm2 感知）
	@$(MAKE) --no-print-directory stop PORT=9999

# start-isolated：以独立数据目录启动一个隔离实例，data dir = ~/.deepthink-<PORT>。
# 用法：
#   make start-isolated PORT=9999
# 说明：强制走非-pm2 直跑路径（pm2 的 deepthink 是单进程托管，无法多开）。
# 独立 data dir → 独立 DB / 用户表 / user-im 飞书配置 / 会话，互不影响。
# 与默认实例（~/.deepthink/data）或其它端口的实例完全隔离。
start-isolated: ## 以独立数据目录启动隔离实例（~/.deepthink-<PORT>，PORT 必填，不走 pm2）
	@if [ "$(origin PORT)" != "command line" ]; then echo "❌ 用法: make start-isolated PORT=9999"; exit 1; fi
	@export DEEPTHINK_DATA_DIR="$(HOME)/.deepthink-$(PORT)"; \
	  echo ">> start-isolated: PORT=$(PORT) DATA_DIR=$$DEEPTHINK_DATA_DIR"; \
...
# stop-isolated：按端口停止隔离实例（杀该端口监听进程，不走 pm2）。
# 用法：make stop-isolated PORT=9999
stop-isolated: ## 停止指定端口的隔离实例（PORT 必填，杀该端口监听进程）
	@if [ "$(origin PORT)" != "command line" ]; then echo "❌ 用法: make stop-isolated PORT=9999"; exit 1; fi
	@if lsof -ti:$(PORT) -sTCP:LISTEN >/dev/null 2>&1; then \
