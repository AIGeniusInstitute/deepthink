.PHONY: dev dev-backend dev-web build build-backend build-web start \
       ...
       desktop-dev desktop-pack-mac desktop-pack-win desktop-pack-linux
...
desktop-pack-mac: desktop-build desktop-fetch-node ## 打包 macOS .dmg
	cd $(DESKTOP_DIR) && npx electron-builder --config build/mac.json

desktop-pack-win: desktop-build ## 打包 Windows .exe（在 Windows runner 上执行）
	TARGET_PLATFORM=win ARCH=x64 node scripts/fetch-node-binary.js
	cd $(DESKTOP_DIR) && npx electron-builder --config build/win.json

desktop-pack-linux: desktop-build ## 打包 Linux AppImage/.deb（在 Linux runner 上执行）
