...
beforeEach(() => {
  tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'deepthink-context-'));
});
...
describe('ClaudeContextResolver', () => {
  ...
  test('plan warns when native CLAUDE.md and DeepThink memory layer are both active', () => {
    const external = path.join(tmp, 'external-claude');
    writeFile(path.join(external, 'CLAUDE.md'), '# admin playbook');
    ...
    const active = buildClaudeContextPlan({ ...base, deepthinkMemoryActive: true });
    expect(active.audit.warnings.some((w) => w.includes('两套全局记忆'))).toBe(true);

    const disabled = buildClaudeContextPlan({ ...base, deepthinkMemoryActive: false });
