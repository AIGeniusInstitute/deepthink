...
const SHARED_TMP =
  process.env.DEEPTHINK_TEST_DATA_DIR ??
  (() => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'deepthink-routes-plugins-'));
...
// flipped per-test via DEEPTHINK_TEST_USER_ROLE env var.
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    c.set('user', {
      id: process.env.DEEPTHINK_TEST_USER_ID ?? 'alice',
      username: 'alice',
      role: process.env.DEEPTHINK_TEST_USER_ROLE ?? 'member',
      permissions: [],
    });
    return next();
  },
...
const USER = 'alice';
process.env.DEEPTHINK_TEST_USER_ID = USER;
...
beforeEach(() => {
  delete process.env.DEEPTHINK_TEST_USER_ROLE;
