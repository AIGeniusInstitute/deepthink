...
const SHARED_TMP =
  ...
  (() => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'deepthink-routes-messages-'));
  ...
  return {
    ...
    authMiddleware: async (c: any, next: any) => {
      c.set('user', {
        ...
        username: 'alice',
        display_name: 'Alice',
        role: (process.env.DEEPTHINK_TEST_USER_ROLE ?? 'member') as 'admin' | 'member',
        permissions: [],
      });
      return next();
    },
...
function asUser(userId: string, role: 'admin' | 'member' = 'member'): void {
  ...
  process.env.DEEPTHINK_TEST_USER_ROLE = role;
}
