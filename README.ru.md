...
- **Autonomy Layer и Autonomous Mode** *(v1.1.0)* — Сквозная Autonomy Layer объединяет 7 capabilities (perception / cognition / decision / execution / learning / adaptation / monitoring) с metrics collection и E2E acceptance; плюс полный Autonomous Mode, позволяющий Агенту завершать задачу end-to-end без человеческого контроля, покрывая three defense layers (CLAUDE.md конституционное переопределение / Supervisor обход уточнения / RLHF вежливость конца хода) и four hard brakes (деструктивные команды / лимит ходов / лимит токенов / обнаружение цикла)
...
- **Cloud-Native и горизонтальное масштабирование** *(v1.4.0)* — PostgreSQL + Redis + MinIO/S3 заменяют одноузловой стек состояния: шина событий Redis для fan-out между подами, распределённые выборы лидера (IM-каналы / планировщик / периодические задачи) и объектное хранилище S3/MinIO для trace-I/O и файлов рабочих пространств. Без `DATABASE_URL` / `REDIS_URL` система деградирует до однопроцессного режима SQLite
- **Agent Group Chat (Swarm)** *(v1.4.0 / v1.5.0)* — Групповые беседы нескольких Agent на основе мест (seats), где каждое место привязывает определение Agent со своим role prompt, политикой высказываний, монтированиями и бюджетом токенов/времени, плюс живая панель выполнения pipeline. **v1.5.0** связывает путь выполнения end-to-end: сообщение группы запускает graph run по местам, ответ каждого места стримится token за token с привязкой к этому месту, а места наследуют Skills / MCP-серверы / базы знаний текущего turn
- **Рабочая среда сотрудничества цифровых сотрудников** *(v1.4.0)* — Постоянные команды цифровых сотрудников с конечным автоматом задач (`pending → in_progress → review → done` и доработка), общей доской (blackboard) и дашбордом пропускной способности
- **AgentNet Disk** *(v1.4.0)* — Корпоративный файловый диск с деревом папок, загрузкой / скачиванием / перемещением / удалением / поиском, корзиной с восстановлением и историей версий файлов
- **Eval Center** *(v1.4.0)* — Автономный продукт оценки на собственном PostgreSQL: проекты → датасеты → версии → тест-кейсы → рубрики → прогоны оценки, с детерминированными проверками, оценкой LLM-судьёй, Golden-аннотациями и обнаружением дрейфа на основе эмбеддингов
...
- **Интернационализация** — 30 языков интерфейса с нативными эндонимами и поддержкой RTL; Agent отвечает на выбранном пользователем языке
...
Для развёртывания с несколькими репликами используйте `make k8s-deploy` — см. раздел Environment Variables в английском README для `DATABASE_URL` / `REDIS_URL`.
...
- **Бэкенд** (Node.js 22 + TypeScript 5.9 + Hono): основной сервис с маршрутизатором сообщений (опрос 2с + дедупликация), очередью конкурентности (до 20 контейнеров + 5 хост-процессов), планировщиком задач (cron / interval / once), WebSocket-сервером для потоковой передачи и терминала, аутентификацией bcrypt + HMAC Cookie, RBAC и управлением конфигурацией с шифрованием AES-256-GCM. Уровень данных — SQLite (режим WAL, схема v1→v70) на одном узле, либо PostgreSQL + pgvector с Redis (шина событий + выборы лидера) и MinIO/S3 (объектное хранилище) при горизонтальном масштабировании в Kubernetes. Также включает слои Harness / Loop Engineering, Agent-as-a-Service (PaaS), Sandbox и Claude Code Plugins.
- **Фронтенд** (`web/`): React 19 + Vite 6 + Zustand 5 + Tailwind CSS 4 SPA, с react-markdown, mermaid, recharts, xterm.js и мобильным PWA.
- **Agent Runner** (`container/agent-runner/`): движок выполнения, работающий внутри Docker-контейнера или как хост-процесс; вызывает `query()` из Claude Agent SDK, эмиттит 30+ типов StreamEvent через stdout и предоставляет 36 инструментов MCP главному процессу через файловые IPC-каналы с атомарной записью.
