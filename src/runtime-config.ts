...
const DANGEROUS_ENV_VARS = new Set([
  ...
  'TMP',
  // DeepThink 内部路径映射
  'DEEPTHINK_WORKSPACE_GROUP',
  'DEEPTHINK_WORKSPACE_GLOBAL',
  'DEEPTHINK_WORKSPACE_IPC',
  'CLAUDE_CONFIG_DIR',
]);
...
function fromStoredProfile(
  ...
  return {
    ...
    anthropicModel: normalizeModel(
      stored.anthropicModel ?? (stored as any).deepthinkModel ?? '',
    ),
...
export function getContainerEnvConfig(folder: string): ContainerEnvConfig {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      ) as ContainerEnvConfig & { deepthinkModel?: string };
      // Backward compat: migrate old field name
      if (
        stored.anthropicModel === undefined &&
        stored.deepthinkModel !== undefined
      ) {
        stored.anthropicModel = stored.deepthinkModel;
        delete stored.deepthinkModel;
      }
      return stored;
    }
  } catch (err) {
...
export interface SystemSettings {
  ...
  // 关闭 admin host 模式下 DeepThink 自带的 memory 注入层（MCP 工具、模板 CLAUDE.md、WORKSPACE_GLOBAL/MEMORY env）
