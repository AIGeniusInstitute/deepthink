...
function getWorkspaceMcpMetaPath(
  ...
  return path.join(getWorkspaceClaudeDir(group), 'deepthink-workspace.json');
}
...
workspaceConfigRoutes.post(
  ...
  async (c) => {
    ...
    if (isUrl) {
      // SSRF 防护：URL 形式必须 HTTPS + 非内网（拒 169.254.169.254、
      // 100.100.100.200 等），并校验 hostname 的 DNS 解析结果。
      ...
      try {
        ...
        return c.json({ error: `Refused skill URL: ${msg}` }, 400);
      }
    }
