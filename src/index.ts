...
function buildWebTraceUrl(folder: string | undefined, turnId?: string): string | null {
  const base = process.env.DEEPTHINK_WEB_URL || process.env.PUBLIC_BASE_URL || process.env.WEB_BASE_URL;
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  if (chatJid === 'web:main' && effectiveGroup.is_home) {
    for (let i = missedMessages.length - 1; i >= 0; i--) {
      ...
      if (!sender || sender === 'deepthink-agent' || sender === '__system__')
        continue;
  ...
  if (isRecovery) {
    ...
    const historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro:
          '检测到本次因切换 provider 需要使用新的底层模型 session。以下是 DeepThink 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      prompt = historyContext.context + prompt;
  ...
  if (!sessionId || willClearSessionOnProviderSwitch(effectiveGroup.folder, agentId)) {
    const historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        maxMessageLength: 700,
        intro:
          '检测到当前 agent 的底层模型 session 是新的（可能因为切换 provider/model 或恢复失败）。以下是 DeepThink 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      prompt = historyContext.context + prompt;
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          // the notify_channels broadcast (AIGeniusInstitute/deep-think#559).
...
async function ensureDockerRunning(): Promise<void> {
  ...
  // Kill and clean up orphaned deepthink containers from previous runs
  try {
    const { stdout } = await execFileAsync(
      'docker',
      ['ps', '--filter', 'name=deepthink-', '--format', '{{.Names}}'],
      { timeout: 10000 },
    );
 ...
 * by a different DeepThink user), re-routing is skipped — the chat stays with
...
function buildOnNewChat(
  ...
  return (chatJid, chatName) => {
    ...
    if (existing) {
      ...
      //      a different DeepThink user → do NOT re-route.
      //
...
main().catch((err) => {
  logger.error({ err }, 'Failed to start deepthink');
  process.exit(1);
});
