...
4. 发布应用版本并等待审批通过
5. 在 DeepThink Web 界面的「设置 → IM 通道 → 飞书」中填入 App ID 和 App Secret
...
2. 记录返回的 Bot Token
3. 在 DeepThink Web 界面的「设置 → IM 通道 → Telegram」中填入 Bot Token
...
4. 在 DeepThink Web 界面的「设置 → IM 通道 → QQ」中填入 App ID 和 App Secret
...
5. 在 DeepThink Web 界面的「设置 → IM 通道 → 钉钉」中填入 Client ID 和 Client Secret
...
1. 在 DeepThink Web 界面的「设置 → IM 通道 → 微信」中开启微信通道
...
make install-host-tools   # 安装宿主机模式所需外部工具（feishu-cli、agent-browser、uv）+ 刷新 builtin-skills 缓存
```

`make install` 会自动修复 node-pty 的 `spawn-helper` 可执行权限（macOS arm64 预构建二进制偶尔缺少 +x 位，会导致 Web 终端 PTY 模式失败）。

#### 日常开发

```bash
make dev              # 前后端并行启动（热更新）；首次自动安装依赖 + 构建容器镜像
make dev-backend      # 仅启动后端（tsx 直跑 TS，无需先 build）
make dev-web          # 仅启动前端（Vite 5173）
make status           # 查看服务运行状态（进程、端口、日志、Docker 容器）
make logs             # 实时查看日志（仅在手动后台化时有效，如 make start > /tmp/deepthink.log 2>&1 &）
make stop             # 停止服务（pm2 托管时走 pm2 stop，否则杀端口监听进程）
```

`make dev` 会自动检测 `package.json` 是否比 `node_modules` 新，是则自动重跑 `make install`；同时确保 Docker 镜像存在（容器模式必备）。如果系统启用了 pm2，会先暂停 pm2 以释放端口，退出时自动恢复。
...
make format-check     # 仅检查格式（CI 用，不修改文件）
make test             # 运行约束测试（vitest，重构前/后必跑）
make sync-types       # 同步 shared/ 下的类型定义到各子项目（stream-event.ts、image-detector.ts）
make clean            # 清理所有构建产物（dist/、web/dist/、container/agent-runner/dist/）
```

> 修改 `shared/stream-event.ts` 后必须跑 `make sync-types` 同步到三个子项目，否则类型不一致。`make build` 和 `make typecheck` 会自动触发同步。

#### 生产部署

```bash
make start            # 一键启动生产环境（前台阻塞运行，日志输出到终端）
make update-sdk       # 手动更新 agent-runner + 主服务的 Claude Agent SDK 到最新版本
make ensure-latest-sdk  # 启动前自动检测 SDK 是否有新版（有才更新，无则跳过；make start 已内置）
```

如需后台运行：`make start > /tmp/deepthink.log 2>&1 &`，然后用 `make logs` 看实时日志、`make status` 查进程状态、`make stop` 停服务。
...
make restore          # 从最近一次备份恢复（或 make restore FILE=xxx.tar.gz 指定文件）
```

> `make reset-init` 会清空 `data/` 整个目录，仅用于测试设置向导或彻底重来。生产环境慎用。

#### 桌面版打包

把 DeepThink 打包成单机可执行应用（macOS `.dmg` / Windows `.exe` / Linux `.AppImage`），详见 `CLAUDE.md` §2.6。

```bash
make desktop-build      # 编译桌面版 Electron 壳（含 build + sync-types + npm install）
make desktop-fetch-node # 拉取当前平台的 Node.js 二进制到 desktop/dev-resources/node（首次打包前必跑）
...
打包产物输出到 `desktop/release/`，包含 `.dmg`/`.exe`/`.AppImage` 主文件和 `.blockmap`（用于增量更新）。

> 跨平台打包必须在对应平台的 runner 上执行（electron-builder 限制）。macOS 可同时打 arm64 + x64 两个架构的 dmg；Windows 和 Linux 各自只能在原生平台执行。

#### 发布 Release

DeepThink 提供两种发布到 GitHub Release 的方式：手动 `make release`（适合单平台快速发布）和 GitHub Actions 全自动（适合正式版本发布，三平台并行构建）。

**方式一：手动发布（`make release`）**

前提：已 `make desktop-pack-*` 生成产物，且已打 tag 并推送到远端。
...
# 3. 发布到 GitHub Release（需先 brew install gh && gh auth login）
...
- 检查 `gh` CLI 是否安装并登录
- 检查本地是否存在对应 tag
- 检查 `desktop/release/` 是否有产物
- 若 `docs/release-notes/v1.0.0.md` 存在则用作 release notes，否则用 `--generate-notes` 自动从 commit history 生成
- 上传 `desktop/release/` 下所有文件作为 release assets
- 标记为 `--latest`（影响 `electron-updater` 的更新检查）

**方式二：GitHub Actions 全自动（`.github/workflows/release.yml`）**

push `v*` 开头的 tag 自动触发，三平台并行构建后自动创建 Release：

```bash
# 1. 打 tag 并推送（workflow 自动触发）
git tag -a v1.0.0 -m "Release v1.0.0"
git push origin v1.0.0
...
也可在 GitHub 仓库 Actions 页面手动 `workflow_dispatch` 触发（需传 version 参数，对应已存在的 tag）。

> 工作流使用 `softprops/action-gh-release@v2`，`generate_release_notes: true` 自动从 commits 生成 release notes。如需自定义，在 push tag 前把内容写到 `docs/release-notes/v1.0.0.md`，手动 `make release` 会优先用该文件（Actions 工作流目前用自动生成）。

#### 帮助

```bash
make help    # 列出所有可用的 make 命令及说明
```
