...
- **Agent Group Chat (Swarm)** *(v1.4.0 / v1.5.0)* — Conversazioni di gruppo multi-Agent basate su seat, dove ogni seat associa una definizione di agent con il proprio prompt di ruolo, policy di intervento, mount e budget di token/tempo, più un pannello di esecuzione pipeline in tempo reale. **v1.5.0** collega il percorso di esecuzione end-to-end: un messaggio del gruppo avvia un graph run sui seat, la risposta di ogni seat viene trasmessa token per token attribuita al suo seat, e i seat ereditano le Skills / server MCP / basi di conoscenza del turno
