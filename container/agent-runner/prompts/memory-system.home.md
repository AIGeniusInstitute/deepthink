...
#### DeepThink 全局记忆（永久）→ 直接编辑 `/workspace/global/CLAUDE.md`

`/workspace/global/CLAUDE.md` 是 DeepThink 的持久记忆文件，不等同于用户原生 `~/.claude/CLAUDE.md` playbook。
...
> - 是 / 可能 → **DeepThink 全局记忆**（编辑 `/workspace/global/CLAUDE.md`）
