...
const MEMORY_FLUSH_ALLOWED_TOOLS = [
  'mcp__deepthink__memory_search',
  'mcp__deepthink__memory_get',
  'mcp__deepthink__memory_append',
...
// mcp__deepthink__* 部分不在这里硬编码，而是在 main() 里按 createMcpTools() 的注册全集
...
function buildPromptAudit(pieces: PromptPiece[]): ClaudeContextAudit['deepthinkPrompt'] {
...
function runtimeContextAuditBase(containerInput: ContainerInput): ClaudeContextAudit {
  return {
    ...
    deepthinkPrompt: containerInput.contextAudit?.deepthinkPrompt ?? { totalBytes: 0, files: [] },
...
function enrichContextAudit(
  ...
  promptAudit: ClaudeContextAudit['deepthinkPrompt'],
...
function formatTranscriptMarkdown(messages: ParsedMessage[], title?: string | null): string {
  ...
  for (const msg of messages) {
    const sender = msg.role === 'user' ? 'User' : 'DeepThink';
...
function buildMemoryRecallPrompt(isHome: boolean, disableMemoryLayer: boolean): string {
  // 禁用记忆层：完全跳过 DeepThink 的记忆系统提示，让用户本机 ~/.claude/ Playbook 接管
...
function loadUserMcpServers(): Record<string, unknown> {
  // 禁用记忆层模式下 CLAUDE_CONFIG_DIR 指向 ~/.claude/，DeepThink 管理的 per-user MCP
  // 不在那份 settings.json 里，container-runner 通过 env 透传。优先读 env。
  const envJson = process.env.DEEPTHINK_USER_MCP_SERVERS_JSON;
  if (envJson) {
    try {
...
async function runQuery(
  ...
  // 调试观察：DEEPTHINK_DUMP_PROMPT=true 时把最终 system prompt 输出到 stderr
  // host 已通过 logs/ 捕获 stderr，方便对比改 prompts/*.md 前后的差异
  if (process.env.DEEPTHINK_DUMP_PROMPT === 'true') {
...
async function main(): Promise<void> {
  ...
  // 禁用 DeepThink 记忆层：不注册 memory MCP 工具，让 Agent 按用户本机 Playbook 行事
  const disableMemoryLayer = process.env.DEEPTHINK_DISABLE_MEMORY_LAYER === 'true';
  ...
  if (containerInput.isScheduledTask) {
    const scheduledTaskPrefixLines = [
      ...
      '',
      '重要：你正在定时任务模式下运行。你的最终输出不会自动发送给用户。你必须使用 mcp__deepthink__send_message 工具来发送消息，否则用户将收不到任何内容。',
      '',
      '注意：只在完成任务后调用一次 send_message 发送最终结果，不要发送中间状态或重复消息。',
      '',
      '此外：本次运行就是该定时任务本身的执行，对应任务已在调度中。即使下面内容里出现「每隔/每天/定期/提醒我」等字样，也不要再调用 mcp__deepthink__schedule_task 创建新的定时任务（除非内容明确要求你另外新建一个不同的任务）。',
    ];
