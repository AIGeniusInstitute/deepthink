...
- **سحابي أصلاً وقابل للتوسّع أفقياً** *(v1.4.0)* — تستبدل PostgreSQL + Redis + MinIO/S3 حزمة الحالة أحادية العقدة: ناقل أحداث Redis للتوزيع عبر الحاويات (pods)، وانتخاب قائد موزّع (قنوات المراسلة الفورية / المجدول / المهام الدورية)، وتخزين كائنات S3/MinIO لإدخال وإخراج التتبّعات وملفات مساحة العمل. عند عدم تعيين `DATABASE_URL` / `REDIS_URL` يتراجع النظام إلى وضع SQLite أحادي العملية
- **محادثة جماعية للوكلاء (Swarm)** *(v1.4.0 / v1.5.0)* — محادثات جماعية متعددة الوكلاء قائمة على المقاعد، حيث يربط كل مقعد تعريف وكيل بموجّه دوره الخاص وسياسة التحدّث ونقاط التركيب وميزانية الرموز/الوقت، إلى جانب لوحة تنفيذ مباشرة لخط الأنابيب. **v1.5.0** يصل مسار التنفيذ من طرف إلى طرف: رسالة المجموعة تُطلق graph run على المقاعد، ويُبَثّ ردّ كل مقعد رمزًا برمز منسوبًا إلى مقعده، وترث المقاعد Skills / خوادم MCP / قواعد المعرفة الخاصة بالدور
- **منصة تعاون الموظفين الرقميين** *(v1.4.0)* — فرق دائمة من الموظفين الرقميين مع آلة حالة المهام (`pending → in_progress → review → done` بالإضافة إلى إعادة العمل)، ولوح مشترك (blackboard)، ولوحة إنتاجية
- **AgentNet Disk** *(v1.4.0)* — قرص ملفات مؤسسي مع شجرة مجلدات، ورفع / تنزيل / نقل / حذف / بحث، وسلة محذوفات مع الاستعادة، وسجل إصدارات الملفات
- **Eval Center** *(v1.4.0)* — تقييم مستقل على قاعدة PostgreSQL خاصة به: مشاريع → مجموعات بيانات → إصدارات → حالات اختبار → معايير تقييم → دورات تقييم، مع تحققات حتمية وتقييم LLM-judge وتعليقات Golden وكشف الانحراف القائم على التضمينات (embeddings)
...
- **مُدول لغوياً** — 30 لغة واجهة بأسماء ذاتية أصلية ودعم RTL؛ يرد الوكيل باللغة التي اختارها المستخدم
...
للنشر متعدد النسخ (multi-replica)، استخدم `make k8s-deploy` — راجع قسم Environment Variables في README الإنجليزي بخصوص `DATABASE_URL` / `REDIS_URL`.
...
- **الخلفية** (Node.js 22 + TypeScript 5.9 + Hono): الخدمة الرئيسية مع موجّه رسائل (اقتراع 2s + إزالة تكرار)، طابور تزامن (حتى 20 حاوية + 5 عمليات مضيف)، مجدول مهام (cron / interval / once)، خادم WebSocket للبث المباشر والطرفية، مصادقة bcrypt + HMAC Cookie، RBAC، وإدارة تهيئة مشفّرة بـ AES-256-GCM. تستخدم طبقة البيانات SQLite (وضع WAL، مخطط v1→v70) على عقدة واحدة، أو PostgreSQL + pgvector مع Redis (ناقل الأحداث + انتخاب القائد) و MinIO/S3 (تخزين الكائنات) عند التوسّع الأفقي على Kubernetes. كما تتضمن طبقات Harness / Loop Engineering و Agent-as-a-Service (PaaS) و Sandbox و Claude Code Plugins.
...
- **مشغّل الوكيل** (`container/agent-runner/`): محرك التنفيذ الذي يعمل داخل حاوية Docker أو كعملية مضيف؛ يستدعي `query()` من Claude Agent SDK، يبث أكثر من 30 نوعًا من StreamEvent عبر stdout، ويقدم 36 أداة MCP للعملية الرئيسية عبر قنوات IPC قائمة على الملفات مع كتابة ذرّية.
