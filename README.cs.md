...
- **Skupinový chat Agent (Swarm)** *(v1.4.0 / v1.5.0)* — Skupinové konverzace více Agent založené na sedadlech, kde každé sedadlo váže definici Agent s vlastním promptem role, politikou mluvení, mounty a rozpočtem tokenů/času, plus živý panel pro spouštění pipeline. **v1.5.0** propojuje cestu vykonávání end-to-end: zpráva ve skupině spustí graph run nad sedadly, odpověď každého sedadla se streamuje token po tokenu s přiřazením k jeho sedadlu a sedadla dědí Skills / MCP servery / znalostní báze daného turn
