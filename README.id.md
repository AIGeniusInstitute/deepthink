...
- **Agent Group Chat (Swarm)** *(v1.4.0 / v1.5.0)* — Percakapan grup multi-Agent berbasis kursi, di mana setiap kursi mengikat definisi Agent dengan prompt peran, kebijakan bicara, mount, dan anggaran token/waktunya sendiri, plus panel eksekusi pipeline langsung. **v1.5.0** menyambungkan jalur eksekusi secara end-to-end: pesan grup memulai graph run atas kursi-kursi tersebut, balasan setiap kursi dialirkan token demi token dengan atribusi ke kursinya, dan kursi mewarisi Skills / MCP server / knowledge base pada turn itu
