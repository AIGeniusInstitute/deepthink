...
- **Agent Group Chat (Swarm)** *(v1.4.0 / v1.5.0)* — Conversas em grupo multi-Agent baseadas em assentos, onde cada assento vincula uma definição de Agent com seu próprio role prompt, política de fala, mounts e orçamento de tokens/tempo, além de um painel de execução de pipeline ao vivo. **v1.5.0** liga o caminho de execução de ponta a ponta: uma mensagem do grupo inicia uma graph run sobre os assentos, a resposta de cada assento é transmitida token a token atribuída ao seu assento, e os assentos herdam as Skills / servidores MCP / bases de conhecimento do turno
