...
## 9. P1 扩展：自主路由 + 审批闭环 + 运行中 re-plan

> 状态：实施中，2026-07-22
> 在 P0（M1-M6 已合并 main）之上增量。三项 P1 能力：

### 9.1 功能点 4：Supervisor 自动路由 `delegate_team` — P1

**描述**：当用户在 Supervisor 已启用的 chat 发送自然语言复杂任务时，预分派 Supervisor（`supervisor.ts:runSupervisorPreDispatch`）输出 `action: 'delegate_team'`，`web.ts` 消费分支直接调 `deps.buildTeam(...)` 自主组建团队并启动 graph run，无需用户显式输入 `/team`。

**验收标准**：
- `SupervisorDecision.action` 新增 `delegate_team`，`parseDecision` 校验通过。
- `web.ts` 在 `clarify` 分支之后新增 `delegate_team` 分支：调 `deps.buildTeam({goalText, ownerUserId, groupFolder, chatJid, userLanguage})`，成功后落一条"已组建团队"消息（带 runId），失败回退到普通 agent 队列。
- 决策依据：Supervisor prompt 增 `delegate_team` 判据（任务复杂、需多角色协作）。

### 9.2 功能点 5：human 节点审批闭环（对话框内按钮表单）— P1

**描述**：graph 的 `human` 节点从 P0 placeholder 升级为真正的审批闭环。节点定义承载审批表单（问题 + 选项按钮），执行时 pause run 并在 DeepThink 对话框消息流里插入一张审批卡（ApprovalCard），用户点击按钮即提交审批结果，后端把 human 节点 mark completed + 注入审批结果到 state + resume run，下游节点从 state 读取审批结果继续。

**验收标准**：
- `GraphNode` 新增字段：`approvalPrompt`（审批问题）、`approvalOptions: {label,value}[]`（选项按钮）、`approvalStateKey`（审批结果写入 state 的 key，默认 `node_<id>_approval`）。
- `graph-runner` human 分支返回 `paused`（output=审批提示文本）。
- `graph-orchestrator` pause 分支：若节点为 human 且有 approvalOptions，调 `deps.storeApprovalCard(chatJid, {runId, nodeId, title, question, options})` 落库一条带 `attachments` 的 AI 消息（持久 + WS 推送），并广播 `human_approval_request` stream 事件。
- 新增 `POST /api/graph/runs/:id/nodes/:nodeId/approve`（body `{optionId, note?}`）：找 latest human node_run（paused）→ `updateGraphNodeRun` 改 `completed` + 写 `output_summary`/`state_patch_json` → 把 statePatch merge 进 `graph_runs.state_json` → `resumeGraphRun`。修复 P0 的 resume 无限 re-pause（human 节点已 completed，`getCompletedGraphNodeIds` 跳过它）。
- 前端：`MessageBubble` 的 `MessageAttachment` 加 `'approval'` 类型分支，渲染 `ApprovalCard` 组件（标题 + 问题 + 按钮组）；`stores/graph.ts` 加 `approveNode(runId, nodeId, {optionId, note})` → POST approve 端点；点击后乐观置灰 + 显示"已选择 X"。
- `shared/stream-event.ts` 加 `human_approval_request` 事件类型 + `approvalRequest` 字段（同步到 src/web/副本）。

### 9.3 功能点 6：运行中动态 re-plan — P1

**描述**：graph 运行中根据 human 审批结果或外部指令，注册新版本定义并把 run 指向新版本，下次 resume 自动用新定义继续，已完成节点 completed-set 不变、新节点被识别为 ready。

**验收标准**：
- 新增 DB 函数 `repointGraphRunDefinition(runId, newVersion)`：更新 `graph_runs.definition_version`（+ `definition_id` 若同 id）。
- 新增 `POST /api/graph/runs/:id/replan`（body `{nodes, edges, stateSchema?}`）：`registerDefinition` 新版本 + `repointGraphRunDefinition` + `resumeGraphRun`。orchestrator/scheduler 零改动（`buildRunContext` 已按 locked version 加载）。
- 约束：新版本节点 id 不得与已完成节点冲突除非有意重跑；新节点前驱须已 completed 或在新版本中。

### 9.4 P1 非目标

- 不做飞书 IM 审批推送（审批闭环只在 DeepThink 对话框内，按用户明确要求）。
- 不做运行中团队自进化、设计态拖拽、循环节点（P2）。
