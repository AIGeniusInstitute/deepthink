...
http://localhost:3000 에 접속해 셋업 마법사를 따르세요: 관리자 생성(기본 계정 없음), Claude API 설정, 필요 시 IM 채널 설정. 모든 설정은 웹 인터페이스에서 처리하며 별도의 설정 파일은 필요 없습니다. API 키는 AES-256-GCM으로 암호화되어 저장됩니다.
