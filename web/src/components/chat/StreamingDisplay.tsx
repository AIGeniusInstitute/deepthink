...
function AgentContextPanel({ audit }: { audit: AgentContextAudit }) {
  ...
  const rows = [
    ...
    {
      label: 'DeepThink prompt',
      value: `${audit.deepthinkPrompt.files.length} 个补丁 · ${audit.deepthinkPrompt.totalBytes.toLocaleString()} bytes`,
      detail: audit.deepthinkPrompt.files.map((file) => file.name).join(', '),
    },
  ];
