...
interface ChatState {
  ...
  switchEngine: (jid: string, engine: 'claude' | 'atomcode' | 'codex' | 'opencode' | 'pi') => Promise<void>;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  switchEngine: async (jid: string, engine: 'claude' | 'atomcode' | 'codex' | 'opencode' | 'pi') => {
    try {
  ...
  handleStreamEvent: (chatJid, event, agentId?) => {
    ...
    if (event.eventType === 'task_notification' && event.taskId) {
      ...
      if (event.isBackground && shouldEmitBackgroundTaskNotice(resolvedTaskId)) {
        ...
        notifyIfHidden(`DeepThink: ${desc} ${status}`, event.taskSummary);
      }
  ...
  loadAgents: async (jid, opts) => {
    ...
    try {
      ...
      set((s) => {
        ...
        try {
          const savedOrder = localStorage.getItem(`deepthink-agent-order-${jid}`);
          if (savedOrder) {
  ...
  reorderConversations: (jid, orderedIds) => {
    ...
    try {
      localStorage.setItem(`deepthink-agent-order-${jid}`, JSON.stringify(orderedIds));
    } catch { /* ignore */ }
  },
