...
- **Agent Group Chat (Swarm)** *(v1.4.0 / v1.5.0)* — Trò chuyện nhóm đa-Agent theo ghế (seat), mỗi ghế gắn một định nghĩa agent với prompt vai trò, chính sách phát biểu, mount và ngân sách token/thời gian riêng, kèm bảng điều khiển thực thi pipeline trực tiếp. **v1.5.0** nối liền đường thực thi từ đầu đến cuối: một tin nhắn nhóm khởi động một graph run trên các ghế, phản hồi của mỗi ghế được truyền token theo token gắn với chính ghế đó, và các ghế kế thừa Skills / MCP server / knowledge base của lượt đó
